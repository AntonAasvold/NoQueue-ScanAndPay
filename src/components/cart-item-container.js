...
export class CartItemContainer extends LitElement {
  ...
  firstUpdated() {
    this.cartItemContainerElementInitialHight = this.shadowRoot.getElementById('cart-item-container').scrollHeight;
    store.subscribe(() => {
    ...
    var observer = new MutationObserver(entries => {
      ...
      let cartItemElements = this.shadowRoot.querySelectorAll('cart-item');
      let cartItemContainerElement = this.shadowRoot.getElementById('cart-item-container');
      ...
      if ( cartItemContainerElement.childElementCount > 8) {
        cartItemElements.forEach(cartItem => {
          cartItem.setAttribute("class", "cart-item-medium");
        })
      }

    });

    observer.observe(this.shadowRoot.getElementById('cart-item-container'), config);
...
customElements.define('cart-item-container', CartItemContainer);